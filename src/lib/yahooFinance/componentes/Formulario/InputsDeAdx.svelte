<script lang="ts">
	...
	$effect(() => {
		if (deriveds.promessaDeDadosDoYahooFinance.ready) {
			const aux = untrack(() => estados.periodosParaAdx);
			estados.periodosParaAdx = aux.filter((numero) => numero < deriveds.velas.length);
		}
	});
</script>
...
<div class="max-w-sm space-y-2">
	...
	<div class="flex flex-col gap-2 items-center">
		...
		{#each estados.periodosParaAdx as _, index (index)}
			<!-- 2. Mudamos esta div para 'flex' para colocar o Input e o Botão lado a lado e alinhados ao centro verticalmente -->
			<div class="flex items-center gap-1 w-full justify-center">
				<Input
					type="number"
					min={1}
					bind:value={estados.periodosParaAdx[index]}
					class="w-20"
					oninput={(e) => {
						const alvo = e.currentTarget;
						estados.periodosParaAdx[index] = alvo.value === '' ? 1 : Number(alvo.value);
					}}
				/>
				...
				<Button
					...
					size="icon"
					onclick={() => {
						estados.periodosParaAdx.splice(index, 1);
					}}
				>
					✕
				</Button>
	...
	<Button
		...
		onclick={() => {
			estados.periodosParaAdx.push(2);
		}}
		size="sm"
	>
		+ ADICIONAR
