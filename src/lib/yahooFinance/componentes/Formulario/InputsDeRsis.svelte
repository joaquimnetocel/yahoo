<script lang="ts">
	...
	// REMOVE OS RSIs IMPOSSÍVEIS DE CALCULAR DEVIDO À QUANTIDADE DE VELAS
	$effect(() => {
		if (deriveds.promessaDeDadosDoYahooFinance.ready) {
			const aux = untrack(() => estados.periodosParaRsis);
			estados.periodosParaRsis = aux.filter((numero) => numero < deriveds.velas.length);
		}
	});
</script>
...
<div class="max-w-sm space-y-2">
	...
	<div class="flex flex-col gap-2 items-center">
		...
		{#each estados.periodosParaRsis as _, index (index)}
			<!-- 2. Mudamos esta div para 'flex' para colocar o Input e o Botão lado a lado e alinhados ao centro verticalmente -->
			<div class="flex items-center gap-1 w-full justify-center">
				<Input
					type="number"
					min={1}
					bind:value={estados.periodosParaRsis[index]}
					class="w-20"
					oninput={(e) => {
						const alvo = e.currentTarget;
						estados.periodosParaRsis[index] = alvo.value === '' ? 1 : Number(alvo.value);
					}}
				/>
				...
				<Button
					...
					size="icon"
					onclick={() => {
						estados.periodosParaRsis.splice(index, 1);
					}}
				>
					✕
				</Button>
	...
	<Button
		...
		onclick={() => {
			estados.periodosParaRsis.push(2);
		}}
		size="sm"
	>
		+ ADICIONAR
